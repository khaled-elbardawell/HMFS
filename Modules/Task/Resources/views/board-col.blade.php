<div class="col-md-3 {{$post_type}}">
    <div class="bg-light p-4">
        ...
        <h4 class="header-title pb-1 mb-3 mt-0">{{$post_type}}</h4>

        <div id="project-list-right" class="pb-1">
            @foreach ($tasks as $task)
            {{-- @dd($post_type) --}}
                @if ($task->post_type == $post_type)
                    @include('task::board-card',['task'=>$task])
                @endif
            @endforeach
        </div><!--end project-list-right-->
        @can('task.create')
            <a href="{{route('task.create',['post_type'=>$post_type])}}" class="btn btn-block btn-gradient-primary">Add Task</a>
        @endcan
    </div><!--end /div-->
</div><!--end col-->
